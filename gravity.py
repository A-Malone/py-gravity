...
class Body(object):
    def __init__(self, obj_id, loc, vel, mass, radius):
        self.obj_id = obj_id
        self.loc = loc
        self.vel = vel
        self.mass = mass
    ...
    def __repr__(self):
        return 'x:{x} y:{y} vx:{vx} vy:{vy} mass:{m} radius:{r}'.format(
            x=self.loc[0], y=self.loc[1], vx=self.vel[0], vy=self.vel[1], m=self.mass, r=self.radius)

    def current_state(self):
        """ Basically a deep copy without the methods """
        return State(self.loc, self.vel, self.mass, self.radius, self.obj_id)
...
class GravSim(threading.Thread):
    ...
    def update_body(self, body, states, dt):
        ...
        # a = body.vel
        # b = self.velocity(body, states, body.loc + a*dt/2, dt/2)
        # c = self.velocity(body, states, body.loc + b*dt/2, dt/2)
        # d = self.velocity(body, states, body.loc + c*dt, dt)
        # body.vel = (a + 2*b + 2*c + d)/6
        # body.loc += body.vel*dt
        #Verlet - 2nd order, symplectic
        # acceleration = self.acceleration(body.loc, body, states);
        # body.loc += dt * (body.vel + dt * acceleration / 2);
        # newAcceleration = self.acceleration(body.loc, body, states);
        # body.vel += dt * (acceleration + newAcceleration) / 2;     
    ...
    def velocity(self, body, states, pos, dt):
        """ Provides slopes for the RK4 integrator """
        a = self.acceleration(pos, body, states)
        return body.vel + a*dt
